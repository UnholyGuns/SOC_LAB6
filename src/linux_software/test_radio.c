...
void print_benchmark(volatile unsigned int *periph_base)
{
    ...
    float time_spent = ((float)(stop_time-start_time))/125000000; //convert clks to time
    ...
    float bytes_transferred = (4*2048); // 32 bit registers = 4 bytes*2048 reads in the loop
    float throughput=(bytes_transferred/1000000)/time_spent; //convert to Mbyte
...
// first, get a pointer to the peripheral base address using /dev/mem and the function mmap
    ...
    printf("\r\n\r\n\r\nLab 6 Tim Ferguson - Custom Peripheral Demonstration\n\r");
